...
scene.add(ambientLight) //simulate "hidden" light

const directionalLight = new THREE.DirectionalLight(0x0ffffc, 0.5)
directionalLight.position.set(1, 0.25, 0) //default from straight down
gui.add(directionalLight, 'intensity').min(0).max(1).step(0.001).name('directionalLight intensity')
scene.add(directionalLight) //simulate sunlight


const hemisphereLight = new THREE.HemisphereLight(0xff0000, 0x0000ff, 0.3)
gui.add(hemisphereLight, 'intensity').min(0).max(1).step(0.001).name('hemisphereLight intensity')
scene.add(hemisphereLight) //simulate sky light
...
const spotLight = new THREE.SpotLight(0x78ff00, 4, 8, Math.PI * 0.1, 0.1, 1) 
spotLight.position.set(0, 2, 3)
gui.add(spotLight, 'intensity').min(0).max(10).step(0.001).name('spotLight intensity')
gui.add(spotLight, 'distance').min(0).max(10).step(0.001).name('spotLight distance')
scene.add(spotLight)
